...
.sidebar {
  width: 260px; background-color: #f2e9dc; display: flex;
  flex-direction: column; justify-content: flex-start;
  padding: 40px 25px; position: fixed; left: 0; top: 0; height: 100vh;
}
.sidebar h2 {
  font-size: 20px; font-weight: 700; margin-bottom: 50px; position: relative;
}
.sidebar h2::before {
  content: "";
  width: 5px; height: 25px; background-color: #f4ce14;
  position: absolute; left: -15px; top: 0; border-radius: 3px;
}
.sidebar .menu { display: flex; flex-direction: column; gap: 15px; }
.menu a {
  text-decoration: none; color: #222; display: flex;
...
.main {
  flex: 1; background-color: #fff; padding: 25px 40px;
  overflow-y: auto; margin-left: 260px;
}
...
<div class="sidebar">
  ...
  <div class="menu">
    ...
    <a href="peserta_tambah.php" class="active"><i class="fas fa-envelope"></i> User</a>
  </div>
</div>
      ...
      <?php
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (mysqli_num_rows($cek) > 0) {
                ...
                mysqli_query($koneksi, "INSERT INTO meetings_participant
